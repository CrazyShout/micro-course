...
function articleCards(l) {
  ...
  const mixed=new Set(l.cards.map(c=>c.status)).size>1;
  const list=ids=>'<ul class="article-card-list">'+ids.map(id=>{const c=byId[id];const scope=mixed?' · '+(c.status==='historical_preview'?'历史深化 / Historical extension':'当前课件与配套 / Current & support'):'';return '<li><a href="cards.html?lesson='+l.id+'#'+id+'"><small>'+esc(id+scope)+'</small>'+'<span class="zh">'+c.question_zh_html+'</span><span class="english">'+c.question_en_html+'</span>'+'</a></li>'}).join('')+'</ul>';
  const later=l.study_route.after_practice.length?'<details class="article-aside"><summary>后续再看其余 '+l.study_route.after_practice.length+' 张 / More cards for later</summary>'+list(l.study_route.after_practice)+'</details>':'';
  return '<section id="article-review" class="article-review" tabindex="-1"><div class="eyebrow">UNDERSTAND → RECALL</div><h2>'+articleTitle({zh:'现在，把理解变成记得住的知识',en:'Turn understanding into recall'})+'</h2><p class="zh">先用这 '+l.study_route.first_pass.length+' 张现有卡检查自己。不要逐字背本文：试着说出结论，以及为什么。</p><p class="english">Start with these '+l.study_route.first_pass.length+' existing cards. Explain the conclusion and its reason, rather than reciting this article.</p><a class="article-cta" href="'+esc(D.courses[l.course].markji_url)+'" target="_blank" rel="noopener">去 Markji 复习 / Open Markji ↗</a><p class="article-caption">打开原牌组后按下方卡号搜索；链接不会自动选卡或更改复习排程。<br>Search these IDs in the original deck; this link does not select cards or change your schedule.</p>'+list(l.study_route.first_pass)+later+'</section>';
}

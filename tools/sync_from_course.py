...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='.export-',dir=ROOT) as tmp:
        out=Path(tmp)
        for name in ['learning.css','learning-demos.js','learning-article.css','learning-article-demos.js','learning-curriculum.css']:
        ...
        article_js=(source/'learning-article.js').read_text().replace('index.html','cards.html').replace('codexing-lessons-v1','micro-course-lessons-v1')
        (out/'learning-article.js').write_text(article_js)
        curriculum_js=(source/'learning-curriculum.js').read_text().replace('index.html','cards.html').replace('LEARNING_GUIDE.md','guide.html')
        (out/'learning-curriculum.js').write_text(curriculum_js)
        ...
        for lesson in learning['lessons']:
            ...
            for figure in lesson.get('article',{}).get('figures',[]):
                rel=Path(figure['src'])
                assert rel.parent==Path('learning/figures') and rel.suffix=='.svg' and '..' not in rel.parts,rel
                dst=out/rel;dst.parent.mkdir(parents=True,exist_ok=True)
                shutil.copy2(source/rel,dst);article_figures.add(str(rel))
        ...
        manifest={'schema_version':1,'source_snapshot':learning['snapshot'],
                  ...
                  'teaching_activities':sum(len(c['activities']) for c in learning['curriculum']['courses'].values()),
                  'activity_items':sum(len(a['items']) for c in learning['curriculum']['courses'].values() for a in c['activities']),
    ...
    article_dest=beginner_dest/'articles';article_dest.mkdir(exist_ok=True)
    for lesson in learning['lessons']:
        if lesson.get('article'):shutil.copy2(source/'learning/articles'/(lesson['id']+'.json'),article_dest/(lesson['id']+'.json'))
    ...
    curriculum_dest=beginner_dest/'curriculum';curriculum_dest.mkdir(exist_ok=True)
    for course in learning['curriculum']['courses']:
        shutil.copy2(source/'learning/curriculum'/(course+'.json'),curriculum_dest/(course+'.json'))
    for name in ['README.md','materials-map.md']:
        if (source/'learning/curriculum'/name).exists():shutil.copy2(source/'learning/curriculum'/name,curriculum_dest/name)
    shutil.copy2(source/'learning/TEACHING_DESIGN.md',beginner_dest/'TEACHING_DESIGN.md')
